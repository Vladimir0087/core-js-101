...
function getPolynom(...num) {
  ...
  return (x) => num.reduce((sum, el, ind) => sum + el * (x ** (num.length - 1 - ind)), 0);
}
...
function logger(func, logFunc) {
  return (...args) => {
    const corArgs = JSON.stringify(args).slice(1, -1);
    logFunc(`${func.name}(${corArgs}) starts`);
    const finall = func(...args);
    logFunc(`${func.name}(${corArgs}) ends`);
    return finall;
  };
}
